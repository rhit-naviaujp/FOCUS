# --------------------------------------------------------------------------------------------------------------
# Reference: https://github.com/MendelXu/zsseg.baseline/blob/master/mask_former/modeling/clip_adapter/adapter.py
#            https://github.com/facebookresearch/ov-seg/tree/main/open_vocab_seg/modeling/clip_adapter
...
class FOCUSCLIPRefiner(CLIPRefiner):
    def __init__(
        self,
        clip_model_name: str,
    ...
    def forward(
        ...
        return text_feature, image_features, unnorm_regions, valid_flag
    ...
    def _preprocess_image(
        ...
        # images = F.interpolate(image.unsqueeze(0).float(), size=(224, 224), mode='bilinear', align_corners=False)

        mask_tensor = torch.sigmoid(mask*100)

        foreground_mask = mask_tensor[0:1, :, :].expand(3, -1, -1)

        background_mask = mask_tensor[1:2, :, :].expand(3, -1, -1)
        ...
        mask = mask[valid]
        if not ((valid==1).sum()==2):
            bin_mask = bin_mask.repeat(2, 1, 1)
        ...
        # crop,mask
        regions = [foreground.unsqueeze(0), background.unsqueeze(0)]
        region_masks = [mask_tensor[0:1, :, :].unsqueeze(0), mask_tensor[1:2, :, :].unsqueeze(0)]
