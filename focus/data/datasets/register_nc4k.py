...
NC4K_COLORS = [k["color"] for k in NC4K_CATEGORIES]

MetadataCatalog.get("nc4k").set(
    stuff_colors=NC4K_COLORS[:],
)




def load_nc4k_panoptic_json(json_file, image_dir, gt_dir, semseg_dir,meta):
    """
    Args:
...
def register_nc4k_panoptic(
    ...
    DatasetCatalog.register(
        panoptic_name,
        lambda: load_nc4k_panoptic_json(
...
_PREDEFINED_SPLITS_NC4K = {
    "nc4k": (
        "NC4K/Imgs", 
        "NC4K/GT", 
        "NC4K/NC4K_SEMANTIC.json",
        "NC4K/annotations_detectron2/GT", 
        "NC4K/NC4K_INSTANCE.json"
    ),
...
def get_metadata():
    ...
    thing_classes = [k["name"] for k in NC4K_CATEGORIES if k["isthing"] == 1]
    thing_colors = [k["color"] for k in NC4K_CATEGORIES if k["isthing"] == 1]
    stuff_classes = [k["name"] for k in NC4K_CATEGORIES]
    stuff_colors = [k["color"] for k in NC4K_CATEGORIES]
    ...
    for i, cat in enumerate(NC4K_CATEGORIES):
        if cat["isthing"]:
...
def register_all_nc4k_panoptic(root):
    metadata = get_metadata()
    for (
        prefix,
        (image_root, panoptic_root ,panoptic_json, semantic_root, instance_json),
    ) in _PREDEFINED_SPLITS_NC4K.items():
        # The "standard" version of COCO panoptic segmentation dataset,
        # e.g. used by Panoptic-DeepLab
        register_nc4k_panoptic(
            prefix,
            metadata,
...
register_all_nc4k_panoptic(_root)
