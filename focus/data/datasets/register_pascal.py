...
PASCAL_COLORS = [k["color"] for k in PASCAL_CATEGORIES]

MetadataCatalog.get("pascal").set(
    stuff_colors=PASCAL_COLORS[:],
)




def load_pascal_panoptic_json(json_file, image_dir, gt_dir, semseg_dir,meta):
    """
    Args:
...
def register_pascal_panoptic(
    ...
    DatasetCatalog.register(
        panoptic_name,
        lambda: load_pascal_panoptic_json(
...
_PREDEFINED_SPLITS_PASCAL = {
    "pascal": (
        "PASCAL/img", 
        "PASCAL/gt", 
        "PASCAL/PASCAL_SEMANTIC.json",
        "PASCAL/annotations_detectron2/gt", 
        "PASCAL/PASCAL_INSTANCE.json"
    ),
...
def get_metadata():
    ...
    stuff_classes = [k["name"] for k in PASCAL_CATEGORIES]
    stuff_colors = [k["color"] for k in PASCAL_CATEGORIES]
    ...
    for i, cat in enumerate(PASCAL_CATEGORIES):
        if cat["isthing"]:
...
def register_all_pascal_panoptic(root):
    metadata = get_metadata()
    for (
        prefix,
        (image_root, panoptic_root ,panoptic_json, semantic_root, instance_json),
    ) in _PREDEFINED_SPLITS_PASCAL.items():
        # The "standard" version of COCO panoptic segmentation dataset,
        # e.g. used by Panoptic-DeepLab
        register_pascal_panoptic(
            prefix,
            metadata,
...
register_all_pascal_panoptic(_root)
