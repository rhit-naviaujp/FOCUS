...
HKUIS_COLORS = [k["color"] for k in HKUIS_CATEGORIES]

MetadataCatalog.get("hkuis").set(
    stuff_colors=HKUIS_COLORS[:],
)




def load_hkuis_panoptic_json(json_file, image_dir, gt_dir, semseg_dir,meta):
    """
    Args:
...
def register_hkuis_panoptic(
    ...
    DatasetCatalog.register(
        panoptic_name,
        lambda: load_hkuis_panoptic_json(
...
_PREDEFINED_SPLITS_HKUIS = {
    "hkuis": (
        "HKU-IS/imgs", 
        "HKU-IS/gt", 
        "HKU-IS/HKUIS_SEMANTIC.json",
        "HKU-IS/annotations_detectron2/gt", 
        "HKU-IS/HKUIS_INSTANCE.json"
    ),
...
def get_metadata():
    ...
    thing_classes = [k["name"] for k in HKUIS_CATEGORIES if k["isthing"] == 1]
    thing_colors = [k["color"] for k in HKUIS_CATEGORIES if k["isthing"] == 1]
    stuff_classes = [k["name"] for k in HKUIS_CATEGORIES]
    stuff_colors = [k["color"] for k in HKUIS_CATEGORIES]
    ...
    for i, cat in enumerate(HKUIS_CATEGORIES):
        if cat["isthing"]:
...
def register_all_hkuis_panoptic(root):
    metadata = get_metadata()
    for (
        prefix,
        (image_root, panoptic_root ,panoptic_json, semantic_root, instance_json),
    ) in _PREDEFINED_SPLITS_HKUIS.items():
        # The "standard" version of COCO panoptic segmentation dataset,
        # e.g. used by Panoptic-DeepLab
        register_hkuis_panoptic(
            prefix,
            metadata,
...
register_all_hkuis_panoptic(_root)
