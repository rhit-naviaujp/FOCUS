# ---------------------------------------------------------------------------------------------
# Reference: https://github.com/NiFangBaAGe/Explicit-Visual-Prompt/blob/latest_branch/utils.py
...
def calc_sod(y_pred, y_true):
    ...
    metric_FM = metrics.Fmeasure()
    metric_WFM = metrics.WeightedFmeasure()
    metric_SM = metrics.Smeasure()
    metric_EM = metrics.Emeasure()
    ...
    return f_max/batchsize, mae, sm, em,fm
...
def calc_f1(pd, gt):
    ...
    auc = roc_auc_score(gt.flatten(), pd.flatten())
    return f1, auc,precision, recall
# def calc_f1(y_pred,y_true):
#     batchsize = 1#y_true.shape[0]
#     with torch.no_grad():
#         # assert y_pred.shape == y_true.shape
#         f1, auc = 0, 0
