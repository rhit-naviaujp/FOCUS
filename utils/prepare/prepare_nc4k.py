...
def create_instance_dataset(dataset_dir, output_dir):
    ...
    """
    image_dir = os.path.join(dataset_dir, f"Imgs")
    ...
    # Save to file
    output_file_path = os.path.join(output_dir, f"NC4K_INSTANCE.json")
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    with open(output_file_path, 'w') as f:
        json.dump(coco_format, f)
    print(f"NC4K in COCO format saved to {output_file_path}")
def create_semantic_dataset(dataset_dir, output_dir):
    ...
    # Save to file
    output_file_path = os.path.join(output_dir, f"NC4K_SEMANTIC.json")
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    with open(output_file_path, 'w') as f:
        json.dump(coco_format, f)
...
if __name__ == "__main__":
    dataset_dir = "datasets/NC4K"  # Update this to the path of your dataset
    output_dir = "datasets/NC4K"  # Update this to your desired output directory
    
    create_instance_dataset(dataset_dir, output_dir)
    create_semantic_dataset(dataset_dir, output_dir)
    annotation_dir = Path(os.path.join(dataset_dir ,  "GT"))
    output_dir = Path(os.path.join(dataset_dir , "annotations_detectron2" , "GT"))
    output_dir.mkdir(parents=True, exist_ok=True)
    for file in tqdm.tqdm(list(annotation_dir.iterdir())):
        output_file = output_dir / file.name
        convert(file, output_file)
    print("Annotations for Detectron2 are prepared")