...
def create_instance_dataset(dataset_dir, output_dir, dataset_type):
    ...
    if (dataset_type == "Test"):
        image_dir = os.path.join(dataset_dir, f"DUTS-TE/DUTS-TE-Image")
        mask_dir = os.path.join(dataset_dir, f"DUTS-TE/DUTS-TE-Mask")
    else:
        image_dir = os.path.join(dataset_dir, f"DUTS-TR/DUTS-TR-Image")
        mask_dir = os.path.join(dataset_dir, f"DUTS-TR/DUTS-TR-Mask")        
    ...
    # Save to file
    output_file_path = os.path.join(output_dir, f"DUTS_{dataset_type}.json")
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    with open(output_file_path, 'w') as f:
        json.dump(coco_format, f)
    print(f"DUTS-{dataset_type} in COCO format saved to {output_file_path}")
def create_semantic_dataset(dataset_dir, output_dir, dataset_type):
    ...
    # Save to file
    output_file_path = os.path.join(output_dir, f"DUTS_Semantic_{dataset_type}.json")
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    with open(output_file_path, 'w') as f:
        json.dump(coco_format, f)
...
if __name__ == "__main__":
    dataset_dir = "datasets/DUTS"  # Update this to the path of your dataset
    output_dir = "datasets/DUTS"  # Update this to your desired output directory
    ...
    for name in ["DUTS-TR", "DUTS-TE"]:
        annotation_dir = Path(os.path.join(dataset_dir , f"{name}/{name}-Mask"))
