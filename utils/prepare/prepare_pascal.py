...
def create_instance_dataset(dataset_dir, output_dir):
    ...
    """
    image_dir = os.path.join(dataset_dir, f"img")
    ...
    # Save to file
    output_file_path = os.path.join(output_dir, f"PASCAL_INSTANCE.json")
...
def create_semantic_dataset(dataset_dir, output_dir):
    ...
    # Save to file
    output_file_path = os.path.join(output_dir, f"PASCAL_SEMANTIC.json")
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    with open(output_file_path, 'w') as f:
        json.dump(coco_format, f)
...
def organize_files(base_dir):
    ...
    for root, dirs, files in os.walk(os.path.join(base_dir,"Imgs")):
        for file in files:
            if file.endswith('.png'):
                shutil.move(os.path.join(root, file), os.path.join(gt_dir, file))
            elif file.endswith('.jpg'):
                shutil.move(os.path.join(root, file), os.path.join(img_dir, file))
...
if __name__ == "__main__":
    dataset_dir = "datasets/PASCAL/"  # Update this to the path of your dataset
    output_dir = "datasets/PASCAL/"  # Update this to your desired output directory
